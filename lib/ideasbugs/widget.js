...
(function () {
  ...
  function form() {
    ...
    if (config.boardId) {
      ...
      boardInput.name = "feedback[board_id]";
      boardInput.value = config.boardId;
  ...
  function submit(form) {
    ...
    if (config.boardId) data.append("feedback[board_id]", config.boardId);
  ...
  function thanks() {
    ...
    setTimeout(function () {
      note.textContent = config.labels.thanks + " " + config.labels.pendingReview;
    }, 0);
